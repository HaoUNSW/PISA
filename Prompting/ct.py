...
def generate_numerical(raw_folder, save_path, mode="test", obs_length=15):
    raw_data = np.load(os.path.join(raw_folder, "ct_raw_" + mode + ".npy"))
    data_x = []
    data_y = []
...
def output_sentence(target_temperature):
    out = f"The temperature will be {target_temperature} degree."

    return out


def input_sentence(temperatures, location_id, start_date, obs_length):
    ...
    num_visits_string = ', '.join(map(str, temperatures))
    out = f"From {start_date}, {start_week_day} to {end_day}, {end_week_day}, the average temperature of region {location_id} was {num_visits_string} degree on each day. What is the temperature going to be on {prediction_day}, {prediction_week_day}?"

    return out


def generate_prompt(raw_folder, save_path, mode="train", obs_length=15, first_day="January 1, 2017"):
    raw_data = np.load(os.path.join(raw_folder, "ct_raw_" + mode + ".npy"))
    data_x_prompt = []
    data_y_prompt = []
    for i in range(len(raw_data)):
    ...
    with open(os.path.join(save_path, mode + "y_prompt.txt"), "w") as f:
